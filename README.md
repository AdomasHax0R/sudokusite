...
- If you open a standalone page like `generated_sudoku.html` directly, difficulty links may not exist as files (because they are generated by `sudoku_app --all`).
- In that standalone case, the browser script falls back to “local difficulty” (mistake limit / scoring) without navigating.

## Typical Stage 3 usage idea

In Stage 3 you’d usually write a console app that asks the user:
...
- `sudoku_generate_puzzle(&puzzle, &solution, difficulty);`
- `sudoku_write_html_page_with_solution(out_path, "style.css", &puzzle, &solution, &theme, difficulty);`
